...
Na primeira parte desse código será apresentado o conceito de regressão do ponto de vista da aprendizagem de máquina. Será apresentado o método de regressão fundamental: a regressão linear. Mostrarei como ajustar um modelo de regressão linear e fazer previsões a partir do modelo.
...
A regressão linear é chamada "linear" pois é considerada que a relação da resposta às variáveis é uma função linear de alguns parâmetros. Os modelos de regressão que não são uma função linear dos parâmetros são chamados de modelos de regressão não-linear. 
...
$\beta_{0}$: constante que representa a interceptação da reta no eixo y, ou seja, é o valor de y quando x é igual a 0; 

$\beta_{1}$: representa a inclinação da reta (coeficiente angular) em relação à variável explicativa;
...
Os resultados do modelo ficaram bem ajustados aos dados observados da variável dependente, indicando que o modelo se ajusta bem aos dados de treinamento. A partir de uma perspectiva de previsão, a regressão linear multivariada se comporta da mesma forma que a simples (de uma variável).

## 2) Avaliando e treinando modelos de regressão

Agora que foram abordados os modelos mais básicos de regressão linear, veremos como avaliar o desempenho dos modelos graficamente, assim como duas métricas básicas para modelos de regressão. Também aprenderemos como treinar um modelo de modo que ele funcione bem para dados não vistos no dataset, e não apenas em dados de treinamento.

### 2.1) Avaliando o modelo de regressão

Aqui visualizaremos o quanto o modelo e suas previsões podem errar. Na sequência faremos o cálculo do quanto um modelo pode errar em média.
...
### 2.2) Raíz da média dos residuais ao quadrado

Em inglês é root-mean-square error (RMSE), que mostra o quanto a previsão do modelo está errando, essa é uma das métricas de acurácia de um modelo de regressão linear.
...
### 2.3) Gerando datasets de treino e teste
...
A seguir utilizaremos o modelo mpg_model para previsões sobre o dataset de treino. 
...
Como esse dataset é pequeno, usaremos a validação cruzada para simular previsões com dados não vistos.
...
A validação cruzada mostrou aqui que um modelo com interação dá  melhores previsões.
...
O nosso modelo aqui acertou próximo de 60% das previsões que fez, essa é a nossa acurácia.
...
### 7.2) Regressão de Poisson e quasipoisson para predizer contagens
...
#### 7.2.1) Modelo para predizer aluguel de bicicletas
...
Legal! Fizemos um modelo quasipoisson para prever contagens! Como no modelo de regressão logística, esperamos um pseudo-R² próximo de 1.

Agora vamos usar o modelo para previsões para o mês de Agosto.
...
# visualizandoas previsões X as observações da variável dependente
...
Aqui iremos modelar o peso médio da folha em uma planta de soja em função do tempo (após o plantio). Como você verá, a planta de soja não cresce a uma taxa constante, a taxa de creciemnto dela é como se houvesse um surto de crescimento, que eventualmente diminui. Assim, o peso da folha não é bem descrito por um modelo linear.

* Para modelar uma variável de forma não linear é só usar a função s(): *y ~ s(x)*
* O modelo aditivo generalizado é dade pela função gam() do pacote mgcv: *gam(formula, family, data)*
...
O modelo aditivo generalizado aprende a função não linear de crescimento das plantas de soja.

## 8) Modelos de árvore de decisão

Neste capítulo, veremos algoritmos de modelagem que não assumem linearidade ou aditividade e que podem aprender tipos limitados de interação entre variáveis de entrada. Esses algoritmos são métodos baseados em árvore, que funcionam combinando conjuntos de árvores de decisão que são aprendidas com os dados de treinamento.

### 8.1) Modelo de random forest para previsão de aluguel de bicicletas

Nesse exercício  contruiremos novamente o modelo para predizer o número de alugueis de bicicleta em uma hora em função do clima, do tipo de dia (se é feriado, dia de trabalho, ou final de semana). Treinaremos o modelo para o mês de Julho.

Usaremos o pacote rangerpara  criar o modelo de random forest. Os argumentos que utilizaremos da função ranger() são:

* *formula*
* *data*
* *num.trees*: número de árvores da "floresta"
* *respect.unordered.factors*: especifica como tratar as variáveis fator não ordenadas, é recomendado configurar o argumento para "order" para modeos de regressão.
...
Muito bom! Criamos um modelo dos dados com um com R². No próximo exercício, você verá como ele funciona nos dados de validação.
...
No exercício anterior, você viu que o modelo de random forest para aluguel de bicicleta ficou melhor do que o modelo  de quasipoisson, quando comparamos o RMSE.

Neste exercício, você visualizará as previsões de agosto do modelo de random forest em função do tempo. O gráfico correspondente do modelo de quasipoisson que você construiu em um exercício anterior está na área de trabalho para você comparar.

Lembre-se de que o modelo de quasipoisson identificou principalmente o padrão de horas lentas e ocupadas no dia, mas subestimou um pouco as demandas de pico. Vamos ver como o modelo de random forest lida com isso.
...
Bom trabalho! O modelo de random forest capturou as variações do dia-a-dia na demanda de pico melhor do que o modelo de quasipoisson, mas ainda subestima a demanda de pico e também superestima a demanda mínima. Portanto, ainda há espaço para melhorias.

### 8.2) One-Hot-Encoding para variáveis categóricas
...
Bom trabalho! Como você viu, o vtreat codifica cores novas como o amarelo que não estava presente nos dados. Isso permite que o modelo aceite esses novos valores sem causar falhas.
...
Neste exercício, você estará pronto para construir um modelo xgboost para prever o número de bicicletas alugadas em uma hora em função do clima, do tipo e da hora do dia. Você treinará o modelo com dados do mês de julho.
...
Você usará o pacote xgboost para ajustar o modelo random forest. A função xgb.cv() usa validação cruzada para estimar o erro de aprendizado fora da amostra à medida que cada nova árvore é adicionada ao modelo. O número apropriado de árvores a ser usado no modelo final é o número que minimiza o RMSE.
...
Muito bom! Na maioria dos casos, ntrees.test é menor que ntrees.train. O erro de treinamento continua diminuindo mesmo depois que o erro de teste começa a aumentar. É importante usar validação cruzada para encontrar o número correto de árvores (conforme determinado pelo ntrees.test) e evitar um modelo de overfit.

#### modelo de xgboost para previsão de aluguel de bicicletas 
...
bike_model_xgb <- xgboost(data = as.matrix(bikesJuly.treat), # training data as matrix
                   ...
                   nrounds = 87,       # number of trees to build
...
Boa! No geral, o gráfico de dispersão parecia muito bom, mas você notou que o modelo fez algumas previsões negativas? No próximo exercício, você comparará o RMSE deste modelo com os modelos de bicicletas anteriores que você construiu.

#### Avaliando o modelo de xgboost para aluguel de bicicletas
...
#### Visualizando o modelo de xgboost para aluguel de bicicletas
...
Bom trabalho! O padrão de aumento de gradiente captura variações de aluguel devido à hora do dia e outros fatores melhor que os modelos anteriores. Parabéns pela conclusão do curso!